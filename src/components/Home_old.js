...
import Chart from './UI/Chart_old';
...
const Home = () => {
    ...
    const loadRepo = async () => {
        ...
        let newRow = [];
        // res.data?.map((item, i) => {
         //   item.commit.map((com, i) => {
         //           row.push({name: item.name, commit: com.commit, date: com.date})
          //  })
        ///});
        // res.data?.map((item,i) => {
        //     row.map((newItem,i) => {
        //         if(item.name === newItem.name) {
        //             newRow.push({date: newItem.date, [item.name]: item.commit, });
        //         }
        //     })
        // })
        // console.log("newRow", newRow);
        res.data.forEach(element => temp.push(...element.commit)
            
        );

        setChartData(prevState=>{
         let t=   [...prevState,...temp];
         return t.sort((a,b)=>(new Date(a.date).getTime())-(new Date(b.date).getTime()))
        });
    }
